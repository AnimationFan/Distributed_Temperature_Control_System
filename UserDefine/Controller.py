...
class TaskList:
    ...
    def getTask(self):#选择队列当中存在计时器的
        best_task=None
        self.task_semaphore.acquire()
        if self.taskList.__len__()>0:
            for task in self.taskList:
                if best_task==None:
                    ...
                    if task.timer!=None:
                        best_task=task
                        break
            self.taskList.remove(best_task)
...
class AirCState(threading.Thread):
    ...
    def checkReach(self):
        ...
        if self.defaultconfig.DefaultModle=="Cold":
            ...
            if self.targetTemp>=self.targetTemp:
                bool_reach=True
        if bool_reach:
            count=models.UserRoom.objects.filter(room=self.roomNum,user_name=self.user).count()
            if count==1:
                target_record=models.UserRoom.objects.get(room=self.roomNum,user_name=self.user)
    ...
    def saveRecord(self):
        ...
        self.checkReach()
        endtime=datetime.datetime.now()
        record=models.UseRecord(begin_time=self.begintime,
                           end_time=endtime,
                           user_name=self.user,
    ...
    def run(self):
        while True:
            ...
            if self.on == True and self.wind >= 1:
                ...
                self.semaphore.release()
            #根据自身温度进行判读
            if self.defaultconfig.DefaultModle=="Cold" and self.on==True:
                if self.temp<self.targetTemp:
                    self.turnOff(self.user)
            if self.defaultconfig.DefaultModle=="Hot" and self.on==True:
                if self.temp>self.targetTemp:
                    self.turnOff(self.user)
            time.sleep(5)
...
class Controller(object):
    ...
    def turnOffAirC(self, userId: str, roomNum: str):
        for airc in self.aircList:
            if airc.roomNum ==roomNum :
                airc.turnOff(roomNum)
                return True
        return False

    def addAirC(self,roomNum):
        airc=models.AirC.objects.filter(room_num=roomNum)
        if airc.__len__()>0:
            return False
        else:
            airc=models.AirC(room_num=roomNum)
            airc.save()
    ...
    def delAirC(self,roomNum):
        ...
        if target_airc:
            ...
            target_airc=None
            if models.AirC.objects.filter(room_num=roomNum).count() ==1:
                target_airc=models.AirC.objects.get(room_num=roomNum)
            if target_airc:
                target_airc.delete()
            return True
        else:
